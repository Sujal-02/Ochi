...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setLastScrollY(currentScrollY <= 0 ? 0 : currentScrollY); // Ensures scrollY is never negative
    };
  ...
  return (
    <div
      className={`w-full fixed z-[999] px-20 py-8 font-['Neue Montreal'] flex justify-between items-center 
        transition-all duration-500 ease-out 
        ${isScrollingUp ? 'transform translate-y-0 opacity-100' : 'transform translate-y-[-100%] opacity-0'} 
        backdrop-blur-md bg-black/30 rounded-b-lg`}
    >
      <div className='logo'>
      ...
      <div className='links flex gap-10'>
        {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map((item, index) => (
          <a key={index} className={`text-md font-light ${index === 4 ? "ml-40" : ""}`}>
            {item}
          </a>
        ))}
      </div>
